...
def configManagement():
    ...
    window.config(bg = 'lightgreen')
    window.resizable(width = False, height = False)
    window.title('Practica 4 - Administración de configuración')

    # Etiquetas de texto
    lbl_ip = Label(window, text = 'Ingresa la dirección IP del router', fg = 'black', font = ('Verdana, 12'), bg = 'lightgreen')
    lbl_ip.place(x = 30, y = 20)

    # Cuadros de texto
    txt_ip = Entry(window, fg = 'black', font = ('Verdana, 11'), width = 30)
    txt_ip.place(x = 30, y = 50)

    # Botones
    btn_gen = Button(window, text = 'Generar archivo de configuración', font = ('Verdana, 11'), bg = 'lightgray', command = lambda:[ generateConfigfile(txt_ip.get()), messagebox.showinfo(message = "Archivo generado exitosamente!!")])
    btn_gen.config(width = 30, height = 1)
    btn_gen.place(x = 30, y = 150)

    btn_ext = Button(window, text = 'Extraer archivo de configuración', font = ('Verdana, 11'), bg = 'lightgray', command = lambda:[ exportConfigfile(txt_ip.get()), messagebox.showinfo(message = "Archivo guardado como startup-config")])
    btn_ext.config(width = 30, height = 1)
    btn_ext.place(x = 30, y = 185)

    btn_imp = Button(window, text = 'Importar archivo de configuración', font = ('Verdana, 11'), bg = 'lightgray', command = lambda:[ importConfigfile(txt_ip.get()), messagebox.showinfo(message = "Archivo importado exitosamente!!")])
    btn_imp.config(width = 30, height = 1)
    btn_imp.place(x = 30, y = 220)
